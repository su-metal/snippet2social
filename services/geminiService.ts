...
const getRelativeLengthRange = (
  ...
  const percentages = RELATIVE_LENGTH_PERCENTAGES[lengthOption] || [
    0.45,
    0.65,
  ];
...
const getPlatformHardLengthTarget = (
  ...
  if (platformId === "twitter") {
    ...
    const max = allowPremiumLong && lengthOption === "long" ? 4000 : baseRange.max;
    return {
      label: strategy.name,
...
LengthOption is the highest priority; do not vary total length for differentiation—use structure and rhythm only.
...
export const generatePost = async (
  ...
  const hardLengthInstruction = getHardLengthTargets(
    platformId,
    lengthOption,
    language,
    strategy,
  ...
  const generateOnce = async (variantIndex: number = 1): Promise<string> => {
    try {
      const response = await client.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: assembleSystemInstruction(variantIndex),
          },
  ...
  if (variantCount > 1) {
    ...
    for (let i = 0; i < variantCount; i += 1) {
      variants.push(await generateOnce(i + 1));
    }
    return variants;
  }
